...
$(TEST_DIRS): | $(clean)
	...
	CGO_ENABLED=1 go test -short -o test-darwin/$(import_path).test -c $(import_path)
...
.PHONY: test-local
test-local: | $(clean)
	CGO_ENABLED=1 go test -short $(TEST_FLAGS) -covermode=count -coverprofile=coverage-local.out -coverpkg=github.com/Netflix/... ./... \
